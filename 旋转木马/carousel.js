...
(function($) {
    ...
    Carousel.prototype = {
        ...
        setPosterPos: function() {
            var self = this;
            var sliceItems = this.posterItems.slice(1), // 获取剩余的li(帧)  去除第一帧,剩余的截取出来
                sliceSize = sliceItems.length / 2, // 获取剩余(帧)个数的一半
                rightSlice = sliceItems.slice(0, sliceSize), // 右边剩余的个数 从0开始截取到获取剩余(帧)个数的一半
                level = Math.floor(this.posterItems.size() / 2), // 定义层级 因为从0开始计算 所以要向下取整
                leftSlice = sliceItems.slice(sliceSize); // 左边剩余的帧数图片
            ...
            var firstLeft = (this.setting.width - rw) / 2; // 第一帧的left值

            var fixOffsetLeft = firstLeft + rw; // 第一帧left的值 + 第一帧自身的宽度

            // 设置右边的位置关系
            rightSlice.each(function(i) {
                level--;
                // 由于有第一帧作为参考,所以必须先乘以比率关系
                ...
                $(this).css({
                    ...
                    width: rw,
                    height: rh,
                    opacity: 1 / (++j), // 1/1=1  1/2=0.5 以此类推
                    // ( ++i 从1开始计算 * gap 算出循环的每一个缝隙的距离) - 循环图片的宽度
                    left: fixOffsetLeft + (++i) * gap - rw,
                    top: self.setVerticalAlign(rh) // top值就是传入的自定义配置参数
                });
            });
            ...
            leftSlice.each(function(i) {
                $(this).css({
                    ...
                    left: i * gap, // 右侧的left值就是从0开始计数 * gap
                    top: self.setVerticalAlign(lh)
                });

                // 设置宽度高度,由小递增到大(右边显示)  由于lw拿的是右边的宽度 所以要除 才能变大
        ...
        setSettingValue: function() {
            ...
            this.nextBtn.css({
                ...
                // 幻灯片的总数 / 2 然后向上取整 zIndex为最高层级
                // 假设 有9张图片 /2 就是4.5 向上取整为5
            ...
            this.posterFirstItem.css({
                ...
                left: w, // left值就是切换按钮的宽度,使其居中显示
                top: 0,
                // 幻灯片的总数 / 2 然后向下取整,因为zIndex是从0开始的
    ...
    // 初始化页面中的集合
    Carousel.init = function(aPoster) {
        var _this = this;
        aPoster.each(function() {
            new _this($(this)); // new 构造函数, 传入集合中自身的每一个包装成jQuery对象
        });
    };
